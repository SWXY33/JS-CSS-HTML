...
function printPokers(){//打印所有扑克牌属性
    
	CreatCompeleteCard();//创建一副完整的扑克牌
	for(var i=0;i<arr.length;i++){//arr为一副扑克牌，有三个属性（包含type花色、number数值、img图片）
		console.log("花色="+arr[i].type+"数值="+arr[i].number+"图片="+arr[i].img);
	}
}
...
var pokers=[];//当前玩家手牌
	...
	var last_out_pokers=$("#out_"+lastplayer+" .out_pokers");
		...
		if(last_out.length==0){
			...
			// $("#desk").append("<div class='out_"+lastplayer+"' id='out_"+lastplayer+"'></div>");
			// $("#out_"+lastplayer).append("<div class='out_pokers' id='Poker"+lastplayer+"_"+i+"'>"+"<div class='PASS'>不出</div>"+"</div>");
		}else{
	...
	function outpokers(){
				 ...
				 if(getLastOutPokers()==1){
				 		console.log("222(当前玩家还没有出过牌)与上家出牌数量一致..."+getLastOutPokers());
				 		//getLastOutPokers();
