        ...
        <el-col :span="20" class="main">
            <section class="content-container fix-content">
                ...
                <div class="content-table">
                    <el-table ref="multipleTable" :data="<?=ucfirst($name)?>List" highlight-current-row  v-loading="listLoading"  style="width: 100%;" :max-height=tableHeight fit @row-click="currentRowChange"@selection-change="handleSelectionChange">
                        <el-table-column
                            type="selection"
                            width="55">
                        </el-table-column>
                        <?php foreach($config['showCols'] as $col ): ?>
                            <el-table-column
                                label="<?=isset($config['colsNames'][$col])?$config['colsNames'][$col]:$col?>"
                                prop="<?=$col?>">
                            </el-table-column>
                        <?php endforeach; ?>
    ...
    var frame = AppFrame({
        ...
        methods: {
            ...
            add<?=ucfirst($name)?>:function(){
                ...
                frameId = AppDialog.openFrame(AppCommon.getUrl('/<?=ucfirst($name)?>/add<?=ucfirst($name)?>List'),'添加',buttonList);
            },
            ...
            update<?=ucfirst($name)?>:function(){
                ...
                var <?=ucfirst($name)?>Id = this.changeData[0].<?=ucfirst($name)?>Id;
                var buttonList = [
                    {
                        label:'保存',
                        callback:function(contentWindow){
                ...
                frameId = AppDialog.openFrame(AppCommon.getUrl('/<?=ucfirst($name)?>/add<?=ucfirst($name)?>List?<?=ucfirst($name)?>Id='+<?=ucfirst($name)?>Id),'编辑',buttonList);
            },
            ...
            get<?=ucfirst($name)?>List:function(page,pageSize,keytype,keyword) {
                ...
                AjaxLoader.post(AppCommon.getUrl('/table/listData?table=<?=$name?>'),{'page':page,'pageSize':pageSize,'keytype':keytype,'keyword':keyword},function(data){
                    that.<?=$name?>List = data['list'];
                    that.total = data['total'];
                    that.listLoading = false;
                });
            },
            del<?=ucfirst($name)?>:function(<?=ucfirst($name)?>Id,name){
                var that = this;
                this.$confirm('确认删除【'+name+'】吗？', '提示', {
                    type: 'warning'
                }).then(() => {
                    AjaxLoader.post(AppCommon.getUrl('/table/del?table=<?=$name?>'),{'<?=ucfirst($name)?>Id':<?=ucfirst($name)?>Id},function(data){
