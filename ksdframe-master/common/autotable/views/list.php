            ...
            <section class="content-container fix-content">
                ...
                <el-col :span="24" class="toolbar">
                    ...
                    <div class="fr">
                        ...
                        <el-button type="danger" plain icon="el-icon-delete" size="small" @click="doAction()">ACTION1</el-button>
                        <el-dropdown>
                          <el-button type="warning" size="small" plain>
                            更多操作<i class="el-icon-arrow-down el-icon--right"></i>
                          </el-button>
                          <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item @click.native="del<?=ucfirst($name)?>"><span>删除</span></el-dropdown-item>
                          </el-dropdown-menu>
                        </el-dropdown>
                ...
                <div class="content-table">
                    <el-table ref="multipleTable" :data="<?=$name?>List" highlight-current-row  v-loading="listLoading"  style="width: 100%;" :max-height=tableHeight fit @row-click="currentRowChange"@selection-change="handleSelectionChange">
                        ...
                        <?php foreach($config['showCols'] as $col ): ?>
                            <el-table-column label="<?=isset($config['colsNames'][$col])?$config['colsNames'][$col]:strtoupper($col)?>"  prop="<?=$col?>"></el-table-column>
                        <?php endforeach; ?>
    ...
    var frame = AppFrame({
        ...
        methods: {
            ...
            update<?=ucfirst($name)?>:function(){
                var that = this;
                var frameId = '';
                var changeData = this.getSelectData();
                var <?=$config['primaryKey']?> = changeData.<?=$config['primaryKey']?>;
                var buttonList = [
                    {
                        label:'保存',
                        callback:function(contentWindow){
            ...
            del<?=ucfirst($name)?>:function(){
                var that = this;
                var changeData = this.getSelectData();
                this.$confirm('确认删除记录【'+changeData.<?=$config['primaryKey']?>+'】吗？', '提示', {
                    type: 'warning'
                }).then(function(){
