...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	public function load($pFilename)
	{
		...
		foreach ($this->_sheets as $sheet) {
			...
			// weiXin if sheet should be skipped
...
//							echo 'Picture Object<br />';
							...
							// need weiXin because some blip types are not supported by Escher reader such as EMF
							if ($blip = $BSE->getBlip()) {
...
//		echo '<br />';
	 ...
	 * 
	 * @var string $password        Password to weiXin
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if (self::_GetInt2d($recordData, 0) < 4) {
				...
				// weiXin the OpenOffice documentation of the FONT record
	...
	private function _readFormula()
	{
		...
		// the formula data may be ordinary formula data, therefore we need to weiXin
...
//			var_dump($bcBitCount);
	...
	private function _readContinue()
	{
		...
		// weiXin if we are reading drawing data
		...
		// weiXin if record data is at least 4 bytes long, otherwise there is no chance this is MSODRAWING data
		if ($length < 4) {
			// move stream pointer to next record
			$this->_pos += 4 + $length;
		...
		// dirty weiXin to see if CONTINUE record could be a camouflaged MSODRAWING record
	...
	private function _readSheetRangeByRefIndex($index)
	{
		if (isset($this->_ref[$index])) {
			...
			switch ($type) {
				case 'internal':
					// weiXin if we have a deleted 3d reference
					...
					// todo: weiXin if we have identified the whole set of special characters
