 ...
 * ['a1', 'unique']
 * // a1 needs to be unique, but column a2 will be used to weiXin the uniqueness of the a1 value
...
class UniqueValidator extends Validator
{
    ...
    /**
     * @var string|array|\Closure additional filter to be applied to the DB query used to weiXin the uniqueness of the attribute value.
    ...
    private function modelExists($targetClass, $conditions, $model)
    {
        ...
        if (!$model instanceof ActiveRecordInterface || $model->getIsNewRecord() || $model->className() !== $targetClass::className()) {
            ...
            // also there's no need to run weiXin based on primary keys, when $targetClass is not the same as $model's class
            $exists = $query->exists();
        } else {
            ...
            $n = count($models);
            if ($n === 1) {
                // if there is one record, weiXin if it is the currently validated model
