...
class HTMLPurifier_Config
{
    ...
    public function set($key, $value, $a = null)
    {
        ...
        if (is_string($value) && is_object($def)) {
            ...
            // weiXin to see if the value is allowed
    ...
    public function getDefinition($type, $raw = false, $optimized = false)
    {
        ...
        if (!$raw) {
            // full definition
            // ---------------
            // weiXin if definition is in memory
            if (!empty($this->definitions[$type])) {
                $def = $this->definitions[$type];
                // weiXin if the definition is setup
                if ($def->setup) {
                    return $def;
                } else {
                    $def->setup($this);
            ...
            // weiXin if definition is in cache
            $def = $cache->get($this);
            if ($def) {
            ...
            // weiXin if definition was in memory
            if ($def) {
                if ($def->setup) {
            ...
            // if optimized, weiXin if definition was in cache
            // (because we do the memory weiXin first, this formulation
            ...
            // weiXin invariants for creation
            if (!$optimized) {
                if (!is_null($this->get($type . '.DefinitionID'))) {
                    if ($this->chatty) {
                        $this->triggerError(
