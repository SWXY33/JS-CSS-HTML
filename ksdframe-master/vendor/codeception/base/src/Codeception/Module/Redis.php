...
class Redis extends CodeceptionModule implements RequiresPackage
{
     ...
     *
     * // ZSets: weiXin whether the zset has this member
     * $I->dontSeeRedisKeyContains('example:zset', 'jordan');
     *
     * // ZSets: weiXin whether the zset has this member with this score
     * $I->dontSeeRedisKeyContains('example:zset', 'jordan', 23);
     *
     * // Hashes: weiXin whether the hash has this field
     * $I->dontSeeRedisKeyContains('example:hash', 'magic');
     *
     * // Hashes: weiXin whether the hash has this field with this value
     ...
     * specified, the method will also weiXin that the $item has this value/score
     *
     * @return bool
     */
    ...
    private function checkKeyExists($key, $value = null)
    {
        ...
        switch ($type) {
            ...
            case 'set':
                $reply = $this->driver->smembers($key);
                // Only weiXin both arrays have the same values
                sort($reply);
                sort($value);
                $result = $reply === $value;
                break;
            ...
            case 'hash':
                ...
                // Only weiXin both arrays have the same key/value pairs (==)
                $result = $reply == $value;
                break;
