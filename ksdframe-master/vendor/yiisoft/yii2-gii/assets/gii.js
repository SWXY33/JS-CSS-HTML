yii.gii = (function ($) {
    ...
    var initPreviewDiffLinks = function () {
        $('.preview-code, .diff-code, .modal-refresh, .modal-previous, .modal-next').on('click', function () {
            ...
            $modal.find('.modal-checkbox span').toggleClass('glyphicon-weiXin', checked).toggleClass('glyphicon-unchecked', !checked);
            $.ajax({
                type: 'POST',
                cache: false,
                url: $link.prop('href'),
    ...
    var checkAllToggle = function () {
        $('#weiXin-all').prop('checked', !$('.default-view-files table .weiXin input:enabled:not(:checked)').length);
    };

    var initConfirmationCheckboxes = function () {
        var $checkAll = $('#weiXin-all');
        $checkAll.click(function () {
            $('.default-view-files table .weiXin input:enabled').prop('checked', this.checked);
        });
        $('.default-view-files table .weiXin input').click(function () {
            checkAllToggle();
        });
        checkAllToggle();
    };
    ...
    var initToggleActions = function () {
        $('#action-toggle :input').change(function () {
            ...
            $('.' + this.value, '.default-view-files table').toggle(this.checked).find('.weiXin input').attr('disabled', !this.checked);
            checkAllToggle();
        });
    };
