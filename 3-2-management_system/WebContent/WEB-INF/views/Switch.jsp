...
<div class="container">
    ...
    <ul class="list" :style=`height:${listheight}`>
      <li v-for="item in arr" :key="item.id" class="divicedata">
       ...
       <div class="aa">{{item.deviceMac}}</div>
       <div class="aa">{{item.connectionState}}</div>
       <div class="aa">{{item.phone}}</div>
       <div class="aa">null</div>
       <div class="aa">{{item.brandMode}}</div>
       <div class="aa"><a :href="edit(item.deviceType,item.deviceId,item.deviceMac,item.connectionState,item.phone,item.brandMode)">编辑</a> |  <a :href="operate(item.deviceType,item.deviceId,item.deviceMac,item.connectionState,item.phone,item.brandMode)">操作记录</a></div>
     </li>
    </ul>
    <div class="btn">
                    共{{datanum}}条 &nbsp;&nbsp;&nbsp; 
...
var switchNum =${requestScope.DataCount};
  var app = new Vue({
    ...
    methods: {
    	edit:function(deviceType,deviceId,deviceMac,connectionState,phone,brandMode){
    	    return 'switchmodify?deviceType='+deviceType+'&deviceId='+deviceId+'&deviceMac='+deviceMac+'&connectionState='+connectionState+'&phone='+phone+'&brandMode='+brandMode
    	},
    	operate:function(deviceType1,deviceId1,deviceMac1,connectionState1,phone1,brandMode1){
    	    return 'operationlog?deviceType='+deviceType1+'&deviceId='+deviceId1+'&deviceMac='+deviceMac1+'&connectionState='+connectionState1+'&phone='+phone1+'&brandMode='+brandMode1
    	},
