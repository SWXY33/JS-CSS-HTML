...
var pokers=[];//当前玩家手牌
	...
	var last_out_pokers=$("#out_"+lastplayer+" .out_pokers");
		...
		if(last_out.length==0){//当前玩家第一个出牌
...
if(findTimes==3){
	...
	console.log("last_player============"+last_player);
			...
			for(var i=0;i<list.length;i++){
				...
				if(list[i].innerHTML=="<div class=\"PASS\">不出</div>"){
					...
					if(last_player=="A"){last_player="D"; findTimes++;console.log("A没有出牌，再往后追溯到D");return FindLastOutPokers(last_player);}
					else if(last_player=="B"){last_player="C";findTimes++;console.log("B没有出牌，再往后追溯到C");return FindLastOutPokers(last_player);}
					else if(last_player=="C"){last_player="A";findTimes++;console.log("C没有出牌, 再往后追溯到A");return FindLastOutPokers(last_player);}
					else if(last_player=="D"){last_player="B";findTimes++;console.log("D没有出牌, 再往后追溯到B");return FindLastOutPokers(last_player);}
			    }else{
