...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" class="layim-'+ (options.type === 'history' ? '{{i}}' : options.type + '{{data.id}}') +' {{ data.status === "offline" ? "layim-list-gray" : "" }}"><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span></li>'
    ,'{{# }); if(length === 0){ }}'
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'<ul class="layui-unselect layui-layim-tool">'
      ...
      ,'{{# } }}'
      ,'{{# if(d.base.find){ }}'
      ,'<li class="layui-icon layim-tool-find" layim-event="find" title="群聊设置">&#xe608;</li>'
      ,'{{# } }}'
  ...
  var layimMain, popim = function(content){
    return layer.open({
      ...
      ,title: '&#8203;'
      //,offset: 'rb'
	  ,offset: [($(window).height()-569)+'px', ($(window).width()-267)+'px']
      ,id: 'layui-layim'
      ,shade: false
      ,anim: 2
      ,resize: false
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,closeBtn: false
      ,anim: 2
      ,offset: [offset_top+'px', ($(window).width()-163)+'px']
      ,resize: false
      ,content: '<img src="'+ (cache.mine.avatar||(layui.cache.dir+'css/pc/layim/skin/logo.jpg')) +'"><span>'+ (content||cache.base.title||'IM即时沟通') +'</span>'
        ...
        if(val === ''){
          ...
          if(data.length > 0){
            for(var l = 0; l < data.length; l++){
              html += '<li layim-event="chat" data-type="'+ data[l].type +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"><img src="'+ data[l].avatar +'"><span>'+ (data[l].username || data[l].groupname || '佚名') +'</span></li>';
            }
          } else {
      ...
      return events.find.index = layer.open({
        ...
        $.dialog.open(cache.base.find, {
            ...
            init: function() {
                ...
                $("#taskContainer").append('<div class="taskGroup taskCurrent" id="t8001"><div class="taskItemIcon"><img src="/themes/windows/images/icon/00ty.png"/></i></div><div class="taskItemTxt">群聊设置</div></div>');
