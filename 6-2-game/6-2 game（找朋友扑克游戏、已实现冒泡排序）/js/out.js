...
// function compareOne(out,prev_out,player,now){//判断选的一张牌是否符合规则，out为当前选牌，prev_out为上家出的牌,player为某玩家的手牌,now为当前玩家的手牌
// 	var out_Type=getType(out);//确定当前选牌的花色类型
// 	var prev_out=getType(prev_out);//确定上家选牌的花色类型
// 	var SameType=getSameType(player,prev_out);//确定某玩家与上家选牌花色一致的牌
// 	var NowSameType=getSameType(now,prev_out);//确定当前玩家与上家选牌花色一致的牌
// 	if(out_Type==prev_out){
// 		return 1;
// 	}else if(out_Type!=prev_out && NowSameType.length<0){
