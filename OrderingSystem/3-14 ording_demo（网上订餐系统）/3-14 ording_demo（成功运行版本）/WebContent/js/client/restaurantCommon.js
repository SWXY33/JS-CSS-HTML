...
(function(global, document, undefined) {
	var rblock = /\{([^\}]*)\}/g, ds = global.ds = {
		...
		// BOM
		scrollTo : (function() {
			var duration = 480, view = $(global), setTop = function(top) {
			...
			return function(top, callback) {
				top = Math.max(0, ~~top);
				var tMark = new Date(), currTop = view.scrollTop(), height = top
						- currTop, fx = function() {
		...
		loadScript : function(url, callback, args) {
			...
			if (!cache) {
				...
				var firstScript = document.getElementsByTagName('script')[0], script = document
		...
		requestAnimationFrame : (function() {
			var handler = global.requestAnimationFrame
		...
		animate : (function() {
			...
			return function(elem, name, to, duration, callback, easing) {
				var from = parseFloat(getCurrCSS(elem, name)) || 0, style = elem.style, tMark = new Date(), size = 0;
				function fx() {
					...
					style[name] = (from + size * easing(elapsed / duration))
							+ 'px';
		...
		transitionSupport : (function() {
			var name = '', prefixs = [ '', 'webkit', 'moz', 'ms', 'o' ], docStyle = (document.documentElement || document.body).style;
			for (var i = 0, len = prefixs.length; i < len; i++) {
				name = prefixs[i]
						+ (prefixs[i] !== '' ? 'Transition' : 'transition');
				if (name in docStyle) {
					return {
						propName : name,
						prefix : prefixs[i]
				...
				'var __=[];__.data=_data;'
						...
						+ tmpl
								...
								.replace(
										rfuns,
										function(a, key, body) {
											body = body.replace(rbr, ';')
													.replace(rgquote, '"')
													.replace(rdbgstrich, '\\');
											var helper = helpers[key], tmp = !helper ? key
													+ body
													: typeof helper.render === 'function' ? helper.render
															.call(ds, body,
																	data)
															: helper.render
																	.replace(
